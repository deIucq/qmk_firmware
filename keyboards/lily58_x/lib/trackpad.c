...
void trackpad_init(void){
    ...
    trackpad_origin[0] = BMPAPI->app.get_vcc_mv();
    BMPAPI->adc.config_vcc_channel(JOYSTICK_Y_PIN, 3300, 0);
    trackpad_origin[1] = BMPAPI->app.get_vcc_mv();
}

int get_r(void){
    BMPAPI->adc.config_vcc_channel(JOYSTICK_X_PIN, 1200, 700);
    int xSensVal = (trackpad_origin[0] - BMPAPI->app.get_vcc_mv()) * (X_INVERT ? -1 : 1);
    BMPAPI->adc.config_vcc_channel(JOYSTICK_Y_PIN, 1200, 700);
    int ySensVal = (trackpad_origin[1] - BMPAPI->app.get_vcc_mv()) * (Y_INVERT ? -1 : 1);
